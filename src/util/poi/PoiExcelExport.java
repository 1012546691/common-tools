	...
	public void wirteExcel(String titleColumn[], String titleName[],
		...
		for (int s = 0; s < dataList.size() / this.sheetSize + 1; s++) {
			...
			try {
				...
				// 写入excel的表头
				Row titleNameRow = workbook.getSheet(
						this.sheetName + "" + (s + 1)).createRow(0);
				// 设置样式
				HSSFCellStyle titleStyle = workbook.createCellStyle();
				titleStyle = (HSSFCellStyle) setFontAndBorder(titleStyle,
						titleFontType, (short) titleFontSize);
				titleStyle = (HSSFCellStyle) setColor(titleStyle,
						titleBackColor, (short) 10);
				...
				// 通过反射获取数据并写入到excel中
				if (dataList != null
						&& (dataList.size() - s * this.sheetSize) >= 0) {
					// 设置样式
					...
					if (titleColumn.length > 0) {
						for (int rowIndex = 1; rowIndex <= this.sheetSize
								&& dataList.size() >= (s * this.sheetSize + rowIndex); rowIndex++) {
							Object obj = dataList.get(rowIndex - 1 + s
									* this.sheetSize); // 获得该对象
							Class clsss = obj.getClass(); // 获得该对对象的class实例
							Row dataRow = workbook.getSheet(
									this.sheetName + "" + (s + 1)).createRow(
									rowIndex);
							for (int columnIndex = 0; columnIndex < titleColumn.length; columnIndex++) {
								String title = titleColumn[columnIndex]
										.toString().trim();
								if (!"".equals(title)) { // 字段不为空
									// 使首字母大写
									String UTitle = Character.toUpperCase(title
											.charAt(0))
									...
									if (data != null && !"".equals(data)) {
										if ("int".equals(returnType)) {
											...
											cell.setCellValue(floatDecimalFormat
													.format(Float
															.parseFloat(data)));
